...
import "github.com/jakew-o-0/MonkeyInterpreter/token"
...
func (l *Lexer) NextToken() token.Token {
    ...
    default:
        if isDigit(l.ch) {  // get number
            tok.Literal = l.readWord(isDigit)
            tok.Type = token.INT
            return tok
        } else if isLetter(l.ch) { // get string
...
func (l *Lexer)readWord(cond func(byte) bool) string {
    position := l.position
    for cond(l.ch) {
        l.readChar()
    }
